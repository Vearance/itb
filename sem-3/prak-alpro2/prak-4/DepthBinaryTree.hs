...
-- depth tree =
--     if (isTreeEmpty == True) then 0
--     else
--         if (isOneElmt == True) then 1
--         else
--             1 + (max (depth left tree) (depth left tree))

depth tree
    | (isTreeEmpty tree == True) = 0
    | (isOneElmt tree == True) = 1
    | otherwise = 1 + (max (depth (left tree)) (depth (right tree)))
