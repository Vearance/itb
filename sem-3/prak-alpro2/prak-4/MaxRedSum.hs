...
-- Fungsi helper: kembalikan pasangan (ambilNode, tidakAmbilNode)
maxRedSumPair :: BinTree Int -> (Int, Int)
maxRedSumPair Empty = (0, 0)
maxRedSumPair (Node v l r) =
  let (ambilL, skipL) = maxRedSumPair l
      (ambilR, skipR) = maxRedSumPair r

      ambilNode = v + skipL + skipR
      skipNode  = max ambilL skipL + max ambilR skipR
  in (ambilNode, skipNode)
...
maxRedSum tree
  | isTreeEmpty tree = 0
  | otherwise =
      let (ambil, skip) = maxRedSumPair tree
      in max ambil skip
