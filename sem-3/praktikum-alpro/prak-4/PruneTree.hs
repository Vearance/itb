...
pruneTree tree d
  | isTreeEmpty tree = Empty
  | d <= 0 = Empty
  | d == 1 = makeBinTree (akar tree) Empty Empty
  | otherwise =
      makeBinTree (akar tree)
                  (pruneTree (left tree) (d-1))
                  (pruneTree (right tree) (d-1))

                        

-- pruneTree tree i 
--     | i == 0 = Empty
--     | isTreeEmpty tree == True = Empty
--         else
--             if i == 1 then tree
--             else
--                 if (isUnerLeft == True) then makeBinTree((akar tree) (pruneTree (left tree) (i-1)) Empty)
--                 else
--                     if (isUnerRight) == True then makeBinTree((akar tree) Empty (pruneTree (right tree) (i-1)))
--                     else
--                         makeBinTree((akar tree) (pruneTree (left tree) (i-1)) (pruneTree (right tree) (i-1)))
