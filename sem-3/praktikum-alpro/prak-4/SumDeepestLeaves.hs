...
sumAtDepth :: BinTree Int -> Int -> Int -> Int
sumAtDepth Empty _ _ = 0
sumAtDepth (Node v l r) depth targetDepth
  | depth == targetDepth && isOneElmt (Node v l r) = v
  | depth >= targetDepth = 0
  | otherwise = sumAtDepth l (depth + 1) targetDepth + sumAtDepth r (depth + 1) targetDepth
