...
static uint32_t resolve_path(char *path, uint32_t start_inode) {
    ...
    for (uint32_t i = 0; i < num_parts; i++) {
        // Check if component is ".." (parent directory)
        if (memcmp(dir_name_parts[i], "..", 2) == 0 && strlen(dir_name_parts[i]) == 2) {
...
// Helper to check if path ends with slash (indicating it's a directory)
static bool path_is_directory(char *path) {
 ...
 * Can be used to:
 * - Move a file/folder to a different location
 * - Rename a file/folder
 * - Both move and rename simultaneously
 * 
 * @param source_path Current path of file/folder
 * @param dest_path New path or name for file/folder
...
int8_t mv_command(char *source_path, char *dest_path, uint32_t current_inode) {
    ...
    uint8_t src_test_buf[BLOCK_SIZE * 2];
    struct EXT2DriverRequest src_test_request = {
        .buf = src_test_buf,
    ...
    // Extract just the directory name to check
    char test_name[256];
    memcpy(test_name, src_filename, strlen(src_filename));
    test_name[strlen(src_filename)] = '\0';
    src_test_request.name = test_name;
    src_test_request.name_len = strlen(test_name);
    
    int32_t is_dir_retcode = 0;
    syscall(SYS_READ_DIR, (uint32_t)&src_test_request, (uint32_t)&is_dir_retcode, 0);
    
    bool source_is_directory = (is_dir_retcode == 0);
    
    // If source is a directory, just rename it in the parent directory
    if (source_is_directory) {
        // Determine destination filename
        uint32_t dest_parent_inode = current_inode;
        char dest_filename[256];
        
        bool dest_has_trailing_slash = path_is_directory(dest_path);
        
        if (dest_has_trailing_slash) {
            // dest_path like "dir2/" - remove trailing slash and use as filename
            char dest_normalized[512];
            uint32_t dest_len = strlen(dest_path);
            memcpy(dest_normalized, dest_path, dest_len);
            dest_normalized[dest_len] = '\0';
            
            if (dest_len > 1 && dest_normalized[dest_len - 1] == '/') {
                dest_normalized[dest_len - 1] = '\0';
            }
            extract_filename(dest_normalized, dest_filename);
        } else {
            // dest_path like "dir2" - use as filename directly
            char dest_dir[512];
            ...
            if (strlen(dest_dir) > 0) {
                ...
                if (dest_parent_inode == 0) {
                    return 3;  // Invalid destination directory
                }
            }
        }
        
        // Use SYS_RENAME syscall to rename the directory
        struct EXT2DriverRequest rename_request = {
            .buf = (uint8_t *)dest_filename,
            .name = src_filename,
            .name_len = strlen(src_filename),
            .parent_inode = src_parent_inode,
            .buffer_size = strlen(dest_filename),
            .is_directory = true
        };
        
        int32_t rename_retcode = 0;
        syscall(SYS_RENAME, (uint32_t)&rename_request, (uint32_t)&rename_retcode, 0);
        
        return rename_retcode;
    }
    
    // Source is a file - use original file copy logic
    uint8_t src_buf[BLOCK_SIZE * 8];
    ...
    if (dest_is_dir) {
        ...
        // Special case: if dest_path is "..", treat as directory
        if ((memcmp(dest_path, "..", 2) == 0 && strlen(dest_path) == 2) ||
            (memcmp(dest_path, "../", 3) == 0)) {
            // Destination is parent directory
            uint32_t parent_inode = resolve_path("..", current_inode);
            if (parent_inode == 0) {
                return 3;
            }
            dest_parent_inode = parent_inode;
            memcpy(dest_filename, src_filename, strlen(src_filename));
            dest_filename[strlen(src_filename)] = '\0';
        } else {
            char dest_dir[512];
    ...
    struct EXT2DriverRequest del_existing = {
        .buf = NULL,
        .name = dest_filename,
    ...
    syscall(SYS_DELETE, (uint32_t)&del_existing, (uint32_t)&del_retcode, 0);
    ...
    // Delete the source file from original location
    ...
    syscall(SYS_DELETE, (uint32_t)&del_request, (uint32_t)&retcode, 0);
