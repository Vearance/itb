...
/**
 * Process Manager State, contain overall process management information
 *
 * @param active_process_count Number of active process currently running
 * @param _process_pid_used    Internal array to keep track used PID
 */
struct ProcessManager
{
  uint32_t active_process_count;
  bool _process_pid_used[PROCESS_COUNT_MAX];
};

extern struct ProcessManager process_manager;
...
/**
 * Execute usermode process from executable filename
 * 
 * @param exec_filename Executable filename to run
 * @param parent_inode  Parent inode where executable is located
 * @param return_code   Pointer to store execution result code
 * @return              Execution result code, 0 if success
 */
 ...
 * @param process_metadata  Pointer to store process metadata
 */
