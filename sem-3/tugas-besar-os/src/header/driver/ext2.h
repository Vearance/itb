...
#define DISK_SPACE (512u * 1024u * 1024u) // 512MB storage.bin size
...
#define TOTAL_BLOCK_COUNT (DISK_SPACE / BLOCK_SIZE)
#define BLOCK_BITMAP_CAPACITY (BLOCK_SIZE * 8u) // blocks covered by a single bitmap block
#define GROUPS_COUNT ((TOTAL_BLOCK_COUNT + BLOCK_BITMAP_CAPACITY - 1u) / BLOCK_BITMAP_CAPACITY)
#define BLOCKS_PER_GROUP BLOCK_BITMAP_CAPACITY // fixed so each bitmap can describe its group
#define BGD_TABLE_START_BLOCK 2u
...
#define BLOCK_POINTERS_PER_BLOCK (BLOCK_SIZE / sizeof(uint32_t)) // 512/4 = 128 pointers per block
#define DIRECT_BLOCK_COUNT 12
#define SINGLE_INDIRECT_INDEX 12
#define DOUBLE_INDIRECT_INDEX 13
#define TRIPLE_INDIRECT_INDEX 14
...
#define BGD_TABLE_BLOCKS ((GROUPS_COUNT * sizeof(struct EXT2BlockGroupDescriptor) + BLOCK_SIZE - 1u) / BLOCK_SIZE)
 ...
 * @return Error code: 0 success - 1 not a folder - 2 not found - 3 parent folder invalid - 4 not enough buffer - -1 unknown
 */
...
/**
 * Read a slice of a file starting at byte offset into caller buffer.
 * Adjusts length if it would pass EOF. Returns 0 on success, error codes like read().
 */
 ...
 * 
 * @param request EXT2DriverRequest with old name, parent_inode, and new name in buf
 ...
 * @param bgd_loaded pointer to flag that tracks whether a bitmap is loaded
 * @return new last bgd
 */
...
/**
 * @brief get physical block from logical block
 * @param node pointer of inode
 * @param logical_block logical block index
 * @return physical block index, UINT32_MAX if not found
 */
uint32_t get_physical_block(struct EXT2Inode *node, uint32_t logical_block);

/**
 * @brief allocate a single data block
 * @param preferred_bgd preferred block group for allocation
 * @return allocated block number, or 0 if no space
 */
uint32_t allocate_single_block(uint32_t preferred_bgd);

/**
 * @brief allocate indirect blocks recursively
 * @param node pointer to inode
 * @param data pointer to data buffer
 * @param logical_start starting logical block index
 * @param blocks_count number of blocks to allocate
 * @param depth indirect level (1=single, 2=double, 3=triple)
 * @param indirect_block_addr address of indirect block to populate
 * @param preferred_bgd preferred block group
...
/**
 * @brief deallocate indirect blocks recursively
 * @param indirect_block_addr address of the indirect block
 * @param depth depth level (1=single, 2=double, 3=triple)
 * @param bitmap block bitmap buffer
 * @param last_bgd pointer to last block group index
 * @param bgd_loaded pointer to bitmap loaded flag
 */
...
/**
 * @brief get inode from inode index
...
/**
 * @brief find a free bit in the bitmap
 * @param bitmap pointer of bitmap
 * @return index of free bit, UINT32_MAX if not found
 */
...
/**
 * @brief get block level based on blocks needed
 * @param blocks_needed number of blocks needed
 * @return block level: 0 = direct, 1 = single indirect, 2 = double indirect
 */
